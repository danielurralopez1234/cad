...
export class AuthenticationService {
  ...
  async onRegisterUsuarioMantenedor(userMantenedor: Usermantenedor) {
    return new Promise(async (resolve, rejected) => {
        await this.afAuth.auth.createUserWithEmailAndPassword(userMantenedor.email, userMantenedor.password).then(async res => {
          userMantenedor.id = res.user.uid;
            await this.usersService.saveUsersMantenedor(userMantenedor);
            resolve(res);
        }).catch(err => rejected(err));
    });
}
